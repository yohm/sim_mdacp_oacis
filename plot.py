...
def create_config(outfile):
    if len(sys.argv) != 5:
        sys.stderr.write("usage: python run.py <density> <temperature> <length> <seed>\n")
    ...
    with open(outfile, 'w') as f:
        f.write(template.format(density,temperature,length))

create_config("langevin.cfg")

def run_simulator(cfgfile, dat_file):
    simulator = os.path.abspath(os.path.dirname(__file__)) + "/../mdacp"
